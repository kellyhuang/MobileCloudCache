...
public class AwsRegisterMsg extends AwsAppBrokerIntfMsg {
	...
	public static enum regMsgErrCode {
		...
		AWS_REG_MSG_USERNAME_REGISTERED(AWS_REG_MSG_BASE_ECODE + 2),
		AWS_LOGIN_MSG_AUTH_SUCCESS(AWS_LOGIN_MSG_BASE_ECODE + 1),
		AWS_LOGIN_MSG_PASSD_MISMATCH(AWS_LOGIN_MSG_BASE_ECODE + 2),
		AWS_LOGIN_MSG_NOT_REGISTERED(AWS_LOGIN_MSG_BASE_ECODE + 3),
	...
	/*
	private static void AwsPopulateUserHashMap(FileInputStream fileStream)
	{
		...
		do {
			try {
				...
				AwsInit.awsRegisteredUserMap.put(userCredentials[0], 
	...
	public static boolean AwsLoadUserDbFile()
	{
		...
		AwsInit.awsRegisteredUserMap = new HashMap<String, String>();
	...
	public static int AwsIsUserRegistered(String uname, String upwd)
	{
		...
		if ((AwsInit.awsRegisteredUserMap.containsKey(uname)) && 
			...
			if (passd.equals(upwd)) { 
				return regMsgErrCode.AWS_LOGIN_MSG_AUTH_SUCCESS.getValue();
			}
			else {
				return regMsgErrCode.AWS_LOGIN_MSG_PASSD_MISMATCH.getValue();
			}
		}
		else {
			return regMsgErrCode.AWS_LOGIN_MSG_NOT_REGISTERED.getValue();
		}
	}
